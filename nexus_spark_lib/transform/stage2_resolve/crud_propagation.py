...
from nexus_spark_lib.models.er_types import ErOperation, ResolutionMethod
...
async def propagate_delete(
    ...
    cdm_entity_id = await delete_by_source(
        conn, tenant_id, connector_id, source_table, source_record_id
    )
    if cdm_entity_id is None:
        logger.debug(
            "DELETE: source not indexed — %s/%s/%s",
            connector_id,
            source_table,
            source_record_id,
        )
    ...
    affected_attrs = await delete_provenance_for_source(
        conn, cdm_entity_id, tenant_id, connector_id, source_record_id
    )
